# $Id$
package Youri::Check::Test::Updates::Source::Sourceforge;

=head1 NAME

Youri::Check::Test::Updates::Source::Sourceforge - Sourceforge updates source
...
Creates and returns a new Youri::Check::Test::Updates::Source::Sourceforge
object.  
...
sub get_package_version {
    ...
    if($response->is_success()) {
        ...
        while (my $token = $parser->get_tag('a')) {
            ...
            next unless $text =~ /^
                \Q$name\E
                [._-]?($Youri::Check::Test::Updates::VERSION_REGEXP)
            ...
            $max = $version if Youri::Check::Test::Updates::is_newer($version, $max);
        }
        return $max if $max;
    }
    return;
